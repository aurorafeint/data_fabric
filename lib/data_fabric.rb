...
module DataFabric
  ...
  def self.activate_default_shard(shards)
    ensure_default_shards
    shards.each_pair do |key, value|
      @default_shards[key.to_s] = value.to_s
    end
  end
  ...
  def self.active_shard(group)
    raise ArgumentError, 'No shard has been activated' unless shards

    shard = shards[group.to_s]
  ...
  def self.shards
    ensure_default_shards
    ensure_setup
    @default_shards.merge(Thread.current[:shards])
  end

  def self.ensure_default_shards
    @default_shards ||= {}
  end
